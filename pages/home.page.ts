import { APP, FLAGS, TOKEN } from '../app';
import '../shoelace-setup';
import type { ShowQrScannerEventDetail, QuestCompletedEventDetail } from '../types';

class HomePage extends HTMLElement {
  private _qrScannerInstance: HTMLElement | null;
  private readonly _handleClick: (event: Event) => void;
  private readonly _handleQuestCompleted: (event: Event) => void;
  private readonly _handleScannerDismissed: () => void;
  private readonly _handleShowQrScanner: (event: Event) => void;
  private readonly _handlePopState: () => void;
  private readonly _handleAppTasksLoaded: () => void;
  private readonly _handleAppModalDismissed: () => void;
  ...
  private _handleAppTasksLoadedEvent(): void {
    console.log("Tasks are loaded, rendering home page with quests!");
    this.render();
  }

  private _handleAppModalDismissedEvent(): void {
      ...
      if (this._qrScannerInstance) {
          (this._qrScannerInstance as any).dismiss(); // Call dismiss on the scanner itself
  ...
  private _handleClickEvent(event: Event): void {
    ...
    const stampItem = target.closest('.stamp-item');
    if (stampItem) {
      const taskIndex = parseInt((stampItem as HTMLElement).dataset.taskIndex || '0', 10);
      this._showChallengeCard(taskIndex);
    }
  }

  private _showChallengeCard(taskIndex: number): void {
    ...
    challengeCard.setAttribute('task-index', taskIndex.toString());
    ...
    challengeCard.setAttribute('is-completed', FLAGS.get(APP._flags, taskIndex).toString());

    this.appendChild(challengeCard);
  }

  private _handleShowQrScannerEvent(event: Event): void {
    ...
    qrScanner.setAttribute('target-task-index', taskIndex.toString());
    document.body.appendChild(qrScanner);
    this._qrScannerInstance = qrScanner;
  }

  private _handleQuestCompletedEvent(event: Event): void {
    const customEvent = event as CustomEvent<QuestCompletedEventDetail>;
    const { taskIndex } = customEvent.detail;
  ...
  private _handleScannerDismissedEvent(): void {
    console.log("QR Scanner dismissed.");
    this._qrScannerInstance = null;
    this._clearModals();
  }

  private _clearModals(): void {
  ...
  render(): void {
    ...
    wrapper.innerHTML = `
      ...
      <div class="container">
        ...
        ` : `
          <p class="loading-message">Loading quests... <sl-spinner style="font-size: 1.5em;"></sl-spinner></p>
        `}
      </div>
    `;
